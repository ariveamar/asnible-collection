# Ansible — Stop Services via Shell Scripts

Stop multiple Linux services by executing shell scripts from defined directory paths on a target server.  
Full STDOUT/STDERR capture, per-script validation, looping over multiple directory paths, post-stop verification, and configurable IP targeting.
...
```
ansible-stop-services/
├── stop_services.yml                               ← Main playbook
├── inventory/
...
└── roles/
    └── stop_services/
        ...
        └── tasks/
            ...
            └── stop_service.yml                    ← Core: validate + execute + stdout + verify
```
...
stop_scripts:
  - label:       "My Service"              # Friendly name shown in all output
    ...
    args:        "--graceful"             # Optional: arguments to pass to script
```

### Execution Settings
| Variable               | Default | Description                                              |
|------------------------|---------|----------------------------------------------------------|
| `stop_wait_seconds`    | `5`     | Seconds to wait after script runs before verifying       |
...
STEP 6   → Show STDERR (if any)              Print any error output from script
STEP 7   → assert rc == 0                    Fail play if script returned non-zero
STEP 8   → pause N seconds                   Wait for service to shut down
STEP 9   → ps aux | grep <process>           Check if process is still running
           → Print final status output        Show "STOPPED" or "STILL DETECTED" with note
```
...
```bash
# Stop all services with default IP
...
ok: [192.168.1.10] =>
  msg:
    ...
    ├───────────────────────────────────────────────────────────────┤
      ...
      Sending SIGTERM to PID 4821...
      Waiting for process to exit...
      Service stopped successfully.
    └───────────────────────────────────────────────────────────────┘

TASK [[ APP SERVICE A ] STEP 9 — Display final stop verification result]
ok: [192.168.1.10] =>
  msg:
    === Process Status for service: App Service A ===
    Timestamp  : 2025-06-01 10:45:22
    Host IP    : 192.168.1.10
    Script     : /opt/services/app-a/stop.sh

    STATUS     : ✅ STOPPED — No process found for 'App Service A'
    PID(s)     : None
    =============================================================
```
...
- `tr -d '[:space:]'` strips all whitespace from the PID output before the empty check — prevents blank newlines from being treated as a found process (false-positive "STILL RUNNING" bug).
- All stdout output lines are captured and displayed individually via `stdout_lines`.
- If `ignore_script_errors: true`, the loop continues even if a script fails — useful for partial shutdowns.
